...
data_dir = "././data/SocaDataset/"  # path of data
...
for file_name in lst_list:
    if not (
        os.path.exists(
            data_dir + "sentinel-ts/" + ".".join(file_name.split(".")[:-1]) + ".h5"
        )
    ):
        pattern = os.path.join(
            data_dir, "sentinel-ts", ".".join(file_name.split(".")[:-1]) + "_*.h5"
        )
    ...
    with h5py.File(
        data_dir + "sentinel-ts/" + ".".join(file_name.split(".")[:-1]) + ".h5", "r"
    ) as file:
        ...
        s1_asc = torch.cat(
            [s1_asc, (s1_asc[:, 0, :, :] / (s1_asc[:, 1, :, :] + 1e-8)).unsqueeze(1)],
            dim=1,
        )
        ...
        s1_des = torch.cat(
            [s1_des, (s1_des[:, 0, :, :] / (s1_des[:, 1, :, :] + 1e-8)).unsqueeze(1)],
            dim=1,
        )
